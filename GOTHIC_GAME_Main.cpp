...
int main()
{
	...
	item niewykonczony_m("Niewykoñczony Miecz"			, 1 , 20 , 0 , 1, 0);
	item cwiekowana_m	("Æwiekowana Maczuga"			, 2 , 25 , 0 , 0, 0);
	item morgensztern	("Morgensztern"					, 3 , 35 , 0 , 0, 0);
	item dlugi_m		("D³ugi Miecz"					, 4 , 40 , 0 , 0, 0);
	item miecz_n		("Miecz Nienawiœci"				, 5 , 44 , 0 , 0, 0);
	item gruchotacz		("Gruchotacz"					, 6 , 53 , 0 , 0, 0);
	item zardzewialy_m	("Zardzewia³y Miecz Oburêczny"	, 7 , 61 , 0 , 0, 0);
	item dwureczniak	("Dwurêczniak"					, 8 , 81 , 0 , 0, 0);
	...
	item zbroja_s		("Zbroja Stra¿nika"				, 14, 0  , 55, 0, 0);
	item ciezka_z		("Ciê¿ka Zbroja Stra¿nika"		, 15, 0  , 70, 0, 0);
	item zbroja_p		("Zbroja z Pancerzy Pe³zaczy"	, 16, 0  , 80, 0, 0);
	item starozytna_z	("Staro¿ytna Zbroja Magiczna"	, 17, 0  , 95, 0, 0);
																					//wskaŸniki do itemów	
	wstary_m = &stary_m;
	...
	postac scierwojad	("Œcierwojad"		, 40 , 8  , 9  , 50 , 0, 0, 0, 0);
	...
	postac zebacz		("Zêbacz"			, 160, 80 , 40 , 220, 0, 0, 0, 0);
	...
	wgomez4 = &gomez4;
	dialog gomez5("To ju¿ wszyscy, z tych wa¿niejszych\n", 0);
	wgomez5 = &gomez5;
	start();
	cin.ignore();
}
